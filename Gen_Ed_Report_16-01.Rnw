...
\definecolor{color2}{RGB}{250,128,114} % Color of the boxes behind the abstract and headings
%\definecolor{color2}{RGB}{178,34,34} % Color of the boxes behind the abstract and headings
...
session <- toLatex(sessionInfo(), citations=FALSE)
session
#toLatex(session)
@
\section{Results}
...
barplot(distribution, ylim=c(0,500), las=1, xlab="Rubric Score on SCI1", ylab="Overall Frequency", axis.lty = 1, col="firebrick")
@
...
library(xtable)
tempTable <- (xtable(table(myData$SCI1),
...
forestplot(textTable, dataTable,
	   ...
	   graphwidth = unit(120, 'mm'),                  # Make the plot 80mm wide
